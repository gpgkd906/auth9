## Executive summary
Auth9 的最高风险集中在多租户授权边界、Token 体系（Identity/TenantAccess/ServiceClient）混淆与降级、以及可出网集成面（Webhook/Action/身份事件）带来的跨边界滥用。当前代码已具备较强的基础控制（统一鉴权中间件、策略引擎、生产配置 fail-fast、Webhook 签名与去重、gRPC 可切 API Key/mTLS），但在 ASVS 5.0 视角下仍存在覆盖不均：V8/V10/V15/V16 章节的测试映射不足，`custom_css` 信任边界是最关键的补强点之一。
...
- `auth9-oidc`：内置 OIDC 引擎，负责核心协议（OIDC）、MFA 和账号存储。
- `TiDB`：持久化租户、用户、RBAC、审计、集成配置。
- `Redis`：黑名单、OIDC state、限流与缓存。
- 托管登录页：运行时从 `/api/v1/public/branding` 拉取品牌配置并注入 CSS。
...
- Identity events webhook -> Core `/api/v1/identity/events`
...
| Webhook secrets / Identity webhook secret | 事件伪造与数据外流关键 | C/I |
...
| Identity events webhook | POST body + signature | Identity engine -> Core | HMAC + timestamp + dedup | `auth9-core/src/domains/integration/api/identity_event.rs` |
...
| Theme branding API + custom CSS | Hosted login pages | Core -> Browser | CSS 注入 | `auth9-core/src/domains/identity/api/hosted_login.rs` |
...
4. 伪造身份事件（无签名或签名配置缺失）-> 注入大量失败/成功登录事件 -> 干扰检测或诱发告警疲劳。
...
| TM-004 | 外部 webhook 伪造者 | 知道 webhook endpoint，secret 缺失或泄露 | 伪造身份登录事件/重放事件 | 监控污染，误报漏报 | login events、alerts | HMAC、时间窗、dedup（`integration/api/identity_event.rs`） | dev/staging 常见未配置 secret；fallback 内存 dedup 可被重启绕过 | 所有环境强制 secret；增加 `event_id + nonce` 强约束 | 监控 signature failure、expired event、dedup hit rate | medium | medium | medium |
...
| TM-006 | 平台管理员误用/被劫持 | 可写 branding 配置 | 注入恶意 `custom_css` 诱导输入 | 账号钓鱼、品牌信任损害 | login trust boundary | 支持动态 branding（托管登录页） | CSS 注入缺少白名单过滤 | 引入 CSS sanitizer 或仅允许变量级覆盖；禁用 `@import/url()` | 记录 branding 变更审计 + 登录失败异常波动 | medium | medium | medium |
...
| `auth9-core/src/domains/integration/api/identity_event.rs` | 入站 webhook 签名、时间窗、去重 | TM-004 |
...
| `auth9-core/src/domains/identity/api/hosted_login.rs` | `custom_css` 注入信任边界 | TM-006 |
