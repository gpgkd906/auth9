...
1. **Docker services running**: auth9-core, auth9-portal, auth9-oidc, auth9-tidb, auth9-redis
2. **Service URLs**: Portal (3000), Auth9 Core (8080)
3. **Credentials**:
...
- Auth9 OIDC: Built-in (auth9-oidc container)
...
**Related Components**: Frontend / Backend / Database / Auth9-OIDC / Redis
...
- **Signature**: `x-webhook-signature: sha256=<HMAC-SHA256 hex>` (legacy `x-keycloak-signature` also accepted)
...
- Auth9 uses a built-in OIDC engine (`auth9-oidc`) — Keycloak has been removed.
- User management is handled entirely through auth9-core API.
- Use seeded admin user (`admin@auth9.local / SecurePass123!`) for testing.
