...
async function aiAgentConfigureService() {
  ...
  try {
    ...
    // 1. 检查是否已有规则
    console.log('检查现有规则...');
    const existingActions = await client.get(`/api/v1/tenants/${TENANT_ID}/actions?trigger_id=post-login`);
    const existingRule = existingActions.data.find(a => a.name === `${serviceName}-access-control`);

    if (existingRule) {
      console.log('规则已存在，更新...');
      await client.patch(`/api/v1/tenants/${TENANT_ID}/actions/${existingRule.id}`, {
        script: generateAccessControlScript(serviceName),
      });
    } else {
      console.log('创建新规则...');
      await client.post(`/api/v1/tenants/${TENANT_ID}/actions`, {
    ...
    const actions = await client.get(`/api/v1/tenants/${TENANT_ID}/actions?trigger_id=post-login`);
