...
collect_keycloak_public_url() {
    ...
    local current="${CONFIGMAP_VALUES[KEYCLOAK_PUBLIC_URL]:-https://idp-auth9.gitski.work}"
    echo "  当前: $current"
...
print_summary() {
    ...
    echo "  Core 公网 URL: ${CONFIGMAP_VALUES[AUTH9_CORE_PUBLIC_URL]:-https://api-auth9.gitski.work}"
    echo "  Portal URL: ${CONFIGMAP_VALUES[AUTH9_PORTAL_URL]:-https://auth9.gitski.work}"
    echo "  Keycloak 公网 URL: ${CONFIGMAP_VALUES[KEYCLOAK_PUBLIC_URL]:-https://idp-auth9.gitski.work}"
...
print_deployment_complete() {
    ...
    if [ -z "$DRY_RUN" ]; then
        ...
        echo ""
        local keycloak_url="${CONFIGMAP_VALUES[KEYCLOAK_PUBLIC_URL]:-https://idp-auth9.gitski.work}"
