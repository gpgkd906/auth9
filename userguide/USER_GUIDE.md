...
- **登录页 Logo**: 用于托管登录页面（推荐 300x100 px）
...
当您有一个新的 B2B 客户（租户）需要接入 Auth9 时：

1.  **创建租户**：
    - 导航到 `Tenants`，点击 `Create Tenant`。
    - 输入客户公司名称（如 `Acme Corp`）和 Slug（如 `acme`）。
    - *可选*：上传客户的 Logo，配置品牌色。

2.  **配置租户策略**：
    - 在租户详情页，进入 `Settings`。
    - 配置 **密码策略**（如要求更高强度的密码）。
    - 配置 **MFA 策略**（如强制管理员开启 MFA）。

3.  **配置身份源 (可选)**：
    - 如果客户希望使用自己的企业 SSO（如 Azure AD 或 Okta）。
    - 导航到 `Settings` > `Identity Providers`。
    - 添加 SAML 或 OIDC 提供商，填入客户提供的元数据。

4.  **创建管理员账号**：
    - 导航到 `Users` > `Invitations`。
    - 发送邀请给客户的管理员邮箱，分配 `admin` 角色。

5.  **交付**：
    - 将租户的登录链接（通常是 `https://auth.yourdomain.com/login?tenant=acme`）发送给客户。

### 16.2 开发者应用接入流程

当内部开发团队需要将新应用接入 Auth9 时：

1.  **注册应用**：
    - 导航到 `Services`，点击 `Register Service`。
    - 输入应用名称（如 `CRM Dashboard`）和 Base URL。
    - 配置 Redirect URIs（如 `https://crm.internal/callback`）。

2.  **获取凭证**：
    - 在 Service 详情页，获取 `Client ID` 和 `Client Secret`。
    - 将凭证安全地分发给开发团队（如通过 1Password 或 Vault）。

3.  **设计角色模型**：
    - 在 Service 详情页，进入 `Roles` 选项卡。
    - 创建应用所需的角色（如 `crm_admin`, `crm_viewer`, `sales_rep`）。
    - 告知开发团队在代码中如何根据这些角色进行权限判断。

4.  **集成测试**：
    - 开发团队使用 OIDC 库集成登录。
    - 在测试环境中验证登录流程和角色获取是否正常。

### 16.3 安全事件响应流程

当收到安全告警（如 `Brute Force Attack`）时：

1.  **评估威胁**：
    - 在 `Security` > `Alerts` 查看告警详情。
    - 检查攻击源 IP 和受影响的用户账号。

2.  **采取行动**：
    - **封禁 IP**：在防火墙或网关层封禁恶意 IP。
    - **锁定账号**：在 `Users` 页面找到受影响用户，暂时禁用账号或重置密码。
    - **强制登出**：对受影响用户执行 `Force Logout`，清除所有活跃会话。

3.  **后续加固**：
    - 检查并加强 `Rate Limiting` 配置。
    - 通知受影响用户修改密码并启用 MFA。
    - 将告警标记为 `Resolved`。
