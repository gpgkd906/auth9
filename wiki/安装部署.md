...
#### 2.9.5 配置 Portal 环境变量

**重要**: 部署 cloudflared 后，需要更新 auth9-portal 的环境变量配置：

```bash
# 更新 ConfigMap（如果使用 ConfigMap）
kubectl edit configmap auth9-config -n auth9
```
...
spec:
  ...
  - name: auth9-portal
    ...
    - name: AUTH9_CORE_URL
      value: "http://auth9-core:8080"
    - name: AUTH9_CORE_PUBLIC_URL
      value: "https://api.auth9.yourdomain.com"
    - name: AUTH9_PORTAL_URL
      value: "https://auth9.yourdomain.com"
```

**配置说明**:

- `AUTH9_CORE_URL`: 集群内 DNS 地址，Portal SSR 使用（95% 流量，< 1ms 延迟）
- `AUTH9_CORE_PUBLIC_URL`: cloudflared tunnel 公网地址，浏览器 OAuth 重定向使用（5% 流量）
- `AUTH9_PORTAL_URL`: Portal 的公网地址，OAuth 回调使用

详细的性能说明请参考 [请求流向说明](请求流向说明.md)。
