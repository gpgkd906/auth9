...
export const abacApi = {
  listPolicies: async (tenantId: string, accessToken?: string): Promise<{ data: AbacPolicyListPayload }> => {
    const response = await fetch(`${API_BASE_URL}/api/v1/tenants/${tenantId}/abac/policies`, {
      headers: getHeaders(accessToken),
    });
    return handleResponse(response);
  },
  ...
  createDraft: async (
    ...
    accessToken?: string
  ): Promise<{ data: { id: string; policy_set_id: string; version_no: number; status: string } }> => {
    const response = await fetch(`${API_BASE_URL}/api/v1/tenants/${tenantId}/abac/policies`, {
      method: "POST",
      headers: getHeaders(accessToken),
      body: JSON.stringify(input),
  ...
  updateDraft: async (
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/tenants/${tenantId}/abac/policies/${versionId}`, {
      method: "PUT",
      headers: getHeaders(accessToken),
      body: JSON.stringify(input),
    });
  ...
  publish: async (
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/tenants/${tenantId}/abac/policies/${versionId}/publish`, {
      method: "POST",
      headers: getHeaders(accessToken),
  ...
  rollback: async (
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/tenants/${tenantId}/abac/policies/${versionId}/rollback`, {
      method: "POST",
      headers: getHeaders(accessToken),
      body: JSON.stringify({ mode }),
    });
  ...
  simulate: async (
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/tenants/${tenantId}/abac/simulate`, {
      method: "POST",
      headers: getHeaders(accessToken),
      body: JSON.stringify(input),
    });
