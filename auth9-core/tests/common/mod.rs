        ...
        Config {
            ...
            database: DatabaseConfig {
                url: std::env::var("DATABASE_URL")
                    .unwrap_or_else(|_| "mysql://root:password@localhost:3306/auth9_test".to_string()),
                max_connections: 5,
                min_connections: 1,
            },
            redis: RedisConfig {
...
pub async fn cleanup_database(pool: &MySqlPool) -> Result<(), sqlx::Error> {
    ...
    sqlx::query("DELETE FROM user_tenant_roles").execute(pool).await?;
    sqlx::query("DELETE FROM role_permissions").execute(pool).await?;
    sqlx::query("DELETE FROM roles").execute(pool).await?;
    sqlx::query("DELETE FROM permissions").execute(pool).await?;
    sqlx::query("DELETE FROM tenant_users").execute(pool).await?;
