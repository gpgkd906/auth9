...
use crate::models::pending_action::{
...
impl PendingActionRepositoryImpl {
    ...
    fn row_to_pending_action(&self, row: &sqlx::mysql::MySqlRow) -> Result<PendingAction> {
        ...
        let action_type = ActionType::from_str_value(&type_str).ok_or_else(|| {
            CredentialError::Internal(anyhow::anyhow!("unknown action type: {}", type_str))
        })?;
        let status_str: String = row.try_get("status")?;
        let status = ActionStatus::from_str_value(&status_str).ok_or_else(|| {
            CredentialError::Internal(anyhow::anyhow!("unknown action status: {}", status_str))
        })?;
