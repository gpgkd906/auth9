...
> 内部调用 `admin_reset_user_password()` 方法，该方法会**临时清除底层 realm 密码策略**
> 再设置密码，然后**恢复原策略**。`temporary: true` 会在底层认证主体上添加
> `UPDATE_PASSWORD` required action，用户通过托管认证链路登录时会被强制修改密码。
>
> **⚠️ 重要**: 必须通过 Auth9 API 设置密码，**不能直接调用底层 Admin API 的
> `reset-password` 端点**——底层 `reset-password` 会强制执行 realm 密码策略，
...
### 密码历史（底层认证引擎管理）
```
密码历史由底层认证引擎的 `passwordHistory(N)` 策略管理。
Auth9 不存储密码历史（无 password_history 表）。
验证方式：通过容器内 Admin API 查看 realm 密码策略字符串。
```
...
1. **Headless Keycloak 架构**：Auth9 不直接处理用户名/密码登录，所有认证通过底层 OIDC 流程。`/api/v1/auth/token` 仅支持 `authorization_code`、`client_credentials`、`refresh_token` grant types，不支持 `password` grant
2. **密码策略同步**：Auth9 配置的策略通过 `KeycloakSyncService` 同步到底层 realm，包括密码复杂度、年龄限制、历史记录和暴力破解防护
3. **密码设置方式**：使用底层用户更新 API（GET-merge-PUT）设置密码，绕过 Keycloak 23.x `reset-password` 端点的已知 bug
4. **登录事件来源**：登录事件通过 ext-event-http SPI Webhook 推送到 auth9-core，而非在 auth9 token 端点直接记录
