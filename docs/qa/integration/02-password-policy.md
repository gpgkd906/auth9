...
> 密码历史检查和账户锁定的高级策略由 Auth9 内置 OIDC 引擎本地管理。
...
> **架构说明**: Auth9 内置 OIDC 引擎已完全接管密码历史检查。`history_count` 策略
> 由 auth9-oidc 本地执行，密码历史存储在 `credentials` 表中。
...
### 验证方式
```bash
# 验证 Auth9 API 返回的密码策略
curl -s "http://localhost:8080/api/v1/tenants/{id}/password-policy" \
  -H "Authorization: Bearer $TOKEN" | jq .
# 预期包含: history_count = 5
```

### 预期结果
- 步骤 2: Auth9 密码策略配置包含 `history_count: 5`
- 步骤 3: Auth9 认证链路拒绝重用密码
...
> **[IMPLEMENTED]** 密码年龄强制执行已实现。
> 详细 QA 测试文档：[integration/28-password-max-age-enforcement.md](./28-password-max-age-enforcement.md)
...
curl -s "http://localhost:8080/api/v1/tenants/{id}/password-policy" \
  -H "Authorization: Bearer $TOKEN" | jq .
# 预期包含: max_age_days = 90
```

### 预期结果
- 步骤 2: Auth9 密码策略配置包含 `max_age_days: 90`
...
> **架构说明**: 账户锁定由 Auth9 内置 OIDC 引擎的暴力破解防护功能本地执行。
> `lockout_threshold` 和 `lockout_duration_mins` 直接在 auth9-oidc 配置中生效。
> 登录事件通过事件兼容入口以 Webhook 方式推送到 auth9-core，再触发安全检测。
...
### 验证方式
```bash
# 验证 Auth9 密码策略中的暴力破解防护配置
curl -s "http://localhost:8080/api/v1/tenants/{id}/password-policy" \
  -H "Authorization: Bearer $TOKEN" | jq '{lockout_threshold, lockout_duration_mins}'
# 预期: lockout_threshold=5, lockout_duration_mins=30
```

### 预期结果
- 步骤 2: Auth9 暴力破解防护已启用，`lockout_threshold=5`, `lockout_duration_mins=30`
- 步骤 4: 托管认证链路返回 `user_disabled` 错误（账户被 Auth9 内置 OIDC 引擎暂时锁定）
- 步骤 5: 登录事件触发 auth9 `login_events` 和 `security_alerts` 记录
...
# 2. 验证底层认证主体的 pending actions 状态
curl -s "http://localhost:8080/api/v1/hosted-login/pending-actions" \
  -H "Authorization: Bearer $IDENTITY_TOKEN" | jq .
# 预期: 包含 UPDATE_PASSWORD action
```
...
### 密码历史
```
密码历史检查已完全由 Auth9 内置 OIDC 引擎本地管理。
`credentials` 表存储当前密码哈希及历史密码记录。
```
...
3. **密码策略执行（高级）**：高级策略（密码历史、账户锁定、密码过期）由 Auth9 内置 OIDC 引擎本地执行
4. **登录事件来源**：登录事件通过事件兼容入口 Webhook 推送到 auth9-core
