...
本功能为平台级全局黑名单，用于拦截已知恶意来源 IP。管理员可在 Portal「设置」→「密码策略」（路由 `/dashboard/settings/security`）中维护黑名单，系统在登录事件命中黑名单时生成 `suspicious_ip` 告警，且 `severity = critical`。
...
### 测试操作流程
1. 在左侧导航点击「设置」
2. 确认设置页中存在「密码策略」入口（对应路由 `/dashboard/settings/security`）并点击
3. 确认页面中存在「恶意 IP 黑名单」卡片
4. 观察黑名单输入框是否加载当前已保存 IP

### 预期结果
- 「设置」入口可见
- 「密码策略」入口可见且可点击（路由 `/dashboard/settings/security`）
- 页面显示「恶意 IP 黑名单」区域
