...
### 初始状态
- 用户点击 SSO 登录后，在外部托管认证页停留超过 5 分钟
...
### 测试操作流程
1. 先通过正常流程设置 `oauth_state` cookie（点击 SSO 登录后不完成认证）
...
- **共同属性**：
  ...
  - `SameSite=Lax`（允许外部授权链路跨站 redirect 回带 cookie）
  - `Max-Age=300`（5 分钟）
