...
Action 管理 API（`/api/v1/services/{id}/actions*`）属于 Portal 管理链路，默认要求 Tenant Access Token 的 audience 在允许列表中（本地默认 `auth9-portal`）。

推荐 token 获取方式：
1. 先使用 Identity Token 调用 `POST /api/v1/auth/tenant-token`
2. 请求体使用 `service_id: "auth9-portal"`
3. 用返回的 `access_token` 调用 Action API

> 不建议用 `service_id=auth9-m2m-test` 或 `auth9-demo` 的 tenant token 直接调用 Action 管理 API；这会因 audience 不匹配产生 401/403，属于测试方法问题，不是 Action Engine 缺陷。
