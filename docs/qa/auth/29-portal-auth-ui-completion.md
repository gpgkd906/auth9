...
- 确认测试用户存在且未启用 MFA（使用 `admin@auth9.local` 或执行 `./scripts/reset-docker.sh` 后创建的用户）
- 确认 Portal 可访问: 浏览器访问 `http://localhost:3000/login`
- **验证管理员凭据**: 默认管理员密码由 `reset-docker.sh` 种子数据设定。如果不确定密码，可以使用 `gen_token.js` 生成有效 token 来验证凭据，或查看 `scripts/reset-docker.sh` 中的种子数据确认密码。

> **注意**: `test@example.com` 不由 `reset-docker.sh` 预置创建。请使用 `admin@auth9.local`（密码: `SecurePass123!`）或其他已有测试用户。如果密码不正确，请执行 `./scripts/reset-docker.sh` 重新初始化种子数据。
...
> **已移除**: 当前 UI 设计中不存在独立的 SSO 邮箱输入框。密码登录表单直接包含邮箱输入框，无需预填充功能。此步骤不适用于当前实现。
...
> **注意**: React Router's `<Form>` component with `required` attributes provides browser-native validation. Playwright automation may not detect native validation popups — verify manually in browser if automated check fails. If browser native validation does not trigger, check whether JavaScript/React form submission logic (e.g., `useFetcher` or `form.submit()`) is bound to the button's `onClick` event, bypassing HTML form validation. React Router's `<Form>` component submits via standard form submission by default, which triggers `required` validation; however, programmatic submission methods like `fetcher.submit()` will skip browser native validation.
...
> **注意**: 如果显示 "Session expired" / "セッションの有効期限が切れました" 而非凭据错误消息，说明前端 `mapSpecialApiError` 未正确区分凭据错误与会话过期。
...
### 步骤 4: 验证未认证访问 [DEFERRED - by design: TOTP setup requires CONFIGURE_TOTP pending action from login flow. Voluntary TOTP setup from account settings is a separate feature.]
