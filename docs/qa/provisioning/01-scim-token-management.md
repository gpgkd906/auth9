...
> **前置条件**: Enterprise SSO Connector 必须在测试前创建。请通过 API 创建 SSO Connector 或运行种子脚本。`sso_connectors` 表不能为空。
...
> Identity Token 仅允许 POST/DELETE 到 `/api/v1/tenants/*`，不允许 GET 子资源。
...
> Identity Token 对此 GET 端点返回 403。
...
**API 测试**:
```bash
# 生成 Tenant Access Token（使用 gen_token.js，从 .env 读取私钥匹配 Docker 容器）
TOKEN=$(node .claude/skills/tools/gen_token.js)
...
| 症状 | 原因 | 解决方法 |
|------|------|---------|
| 403 "Identity token is only allowed for tenant selection and exchange" | 使用了 Identity Token 而非 Tenant Access Token | 使用 `node .claude/skills/tools/gen_token.js` 生成 Tenant Access Token |
