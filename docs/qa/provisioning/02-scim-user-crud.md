...
在重复执行场景前，先确认目标邮箱和 externalId 尚未被此前测试占用：

```sql
SELECT id, email, scim_external_id
FROM users
WHERE email = 'jane.doe@example.com'
   OR scim_external_id = 'okta-user-001';
-- 预期: 0 行
```

若已存在记录，先删除旧用户或改用新的唯一值（例如带时间戳的 email / externalId），否则服务会按冲突返回 `409 Conflict`。
...
| 现象 | 原因 | 解决 |
|------|------|------|
| 返回 `409`，但响应体看起来像创建成功 | 该 email 或 externalId 已存在，命中了冲突路径 | 先清理旧数据，或改用新的唯一 email / externalId |
