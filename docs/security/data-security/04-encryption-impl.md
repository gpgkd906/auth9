...
> **重要 - 正确的测试端点**: `GET /api/v1/system/email` 返回的是 **masked** 数据（密码字段显示为 `***`），该端点**不执行解密操作**，因此无法用于验证 GCM 篡改检测。正确的测试方式是：
> 1. 篡改数据库中的加密值后，调用 `PUT /api/v1/system/email`（触发读取并解密配置），或
> 2. 调用 `POST /api/v1/system/email/test`（发送测试邮件，需要解密 SMTP 密码），或
> 3. 编写单元测试直接验证 `crypto::aes::decrypt()` 对篡改密文的处理
...
4. 截断密文（移除认证标签）
5. 替换为完全不同的密文
6. 通过**触发解密的端点**（非 masked 端点）检查是否检测到篡改
...
### 修复状态
- ✅ 应用启动时验证密钥长度为 32 字节（已实现：`server/mod.rs` 区分"未设置"与"设置但无效"，后者导致启动失败并给出明确提示）
- ✅ 空密钥视为未配置，禁用加密功能并记录日志
...
|---|------|------|----------|----------|----------|
| 1 | Nonce 重用检测 | ✅ PASS | 2026-03-21 | QA | 无问题 |
| 2 | 密文篡改与认证标签验证 | ✅ PASS | 2026-03-21 | QA | 无问题 |
| 3 | 加密密钥强度与管理 | ✅ PASS | 2026-03-21 | QA | 无问题 |
...
### 执行清单
- [x] M-DATA-04-C01 | 控制: V11.1 | 任务: #20 | 动作: 执行文档内相关攻击步骤并记录证据
- [x] M-DATA-04-C02 | 控制: V11.2 | 任务: #20 | 动作: 执行文档内相关攻击步骤并记录证据
- [x] M-DATA-04-C03 | 控制: V11.4 | 任务: #20 | 动作: 执行文档内相关攻击步骤并记录证据

### 回归记录表
| 检查项ID | 执行结果(pass/fail) | 风险等级 | 证据（请求/响应/日志/截图） | 备注 |
|---|---|---|---|---|
| M-DATA-04-C01 | PASS | 高 | `cargo test crypto::aes::tests` 14项全部通过; `OsRng` CSPRNG生成12字节nonce | Nonce唯一性验证 |
| M-DATA-04-C02 | PASS | 高 | `scripts/qa/test_tampering_decrypt.rs` 验证bit-flip/截断/nonce修改/伪造密文均被GCM认证检测 | GCM认证标签验证 |
| M-DATA-04-C03 | PASS | 高 | docker-compose.yml `${SETTINGS_ENCRYPTION_KEY:?}` 必填; .env密钥32字节且gitignored; 历史泄露已修复(e1e9797) | 密钥强度与管理 |
