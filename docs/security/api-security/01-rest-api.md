...
> **⚠️ 测试前提**: `admin@auth9.local` 是默认平台管理员（platform admin），拥有超级权限，会绕过角色检查。要验证 viewer 角色被正确拒绝访问敏感端点，**必须使用非 `admin@auth9.local` 的用户**获取 `$USER_TOKEN`。用 `admin@auth9.local` 测试低权限拒绝场景将导致误报（返回 200 而非预期的 403）。
