...
### 目的
验证注册页和忘记密码页与登录页保持相同的布局框架和设计语言，且 Dark Mode 下信息层级清晰。
...
- **忘记密码页**:
  ...
  - 成功态: 保持同一张认证卡片，显示成功标题、说明文案、"Try again" 链接和返回登录按钮。
  - 安全设计: 始终返回成功（不泄露邮箱是否注册）。
  - Dark Mode: 卡片、输入框、辅助文案和链接层级清晰，不出现整体发灰。
- **一致性检查**:
  - 三个页面 Card 宽度均为 `max-w-md`（448px）。
  - 均使用 `animate-fade-in-up` 入场动画。
  - 均有 `page-backdrop` 动态背景。
  - 登录页、注册页、忘记密码页右上角均显示语言切换与主题切换控件。
  - 忘记密码页与重置密码页使用同一套独立认证页壳层和品牌标识位置。
