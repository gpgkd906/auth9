...
1. 先确认 `auth9-core` 与 `auth9-portal` 都为 `healthy`。
2. 确认已完成租户选择（不在 `/tenant/select` 或错误页）。
3. 若页面返回 500，先检查后端连通性与 token 状态，再判定为 UI 视觉缺陷。
...
## 场景 5：容器边界安全边距 (Safe Area Clearance)

### 目的
验证所有操作按钮、表单项与容器（Card, Dialog, Screen）边缘保持至少 16-24px 的安全间距，避免视觉粘连。

### 预期视觉效果
- **底部边距**: 卡片底部的操作按钮下方必须留有至少 `pb-6` (24px) 或 `mb-6` 的间距，严禁紧贴卡片边框。
- **分区间距**: 搜索栏区域与下方的列表区域之间，必须有明显的垂直间距（建议 24px），确保功能区边界清晰。
- **窄屏边距**: 在移动端，全宽按钮与屏幕边缘应保持至少 16px 的 `padding`，避免产生“溢出”感。
