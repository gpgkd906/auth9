...
> **⚠️ 实现值与理想值对照**：以下列出实际组件实现值，部分低于 WCAG 推荐的 44px 最小触摸目标。标注为"实现值"的是当前代码中的实际尺寸。

**按钮**：
- Default size 高度：**40px**（`h-10`）— 实现值
- SM size：`h-10` + `min-h-[44px]` = 渲染 **44px**
- LG size / Icon size：**44px**（`h-11`，`min-h-[44px]`）
- 图标按钮最小宽度：44px（`min-w-[44px]`）
- 内边距：`px-5 py-2.5`（default），`px-3`（sm），`px-7`（lg）
...
**表单元素**：
- 输入框高度：**40px**（`h-10`）— 实现值（非 `h-11`）
- Select 触发器高度：**40px**（`h-10`）— 实现值（非 `h-12`）
- 复选框/单选框：**16×16px**（`h-4 w-4`）— 实现值（非 `w-6 h-6`）
- Switch：20×36px（`h-5 w-9`）

> **触摸目标合规性说明**：
> - Input/Select 的 40px 低于 WCAG 推荐 44px，但在移动端的内边距 + 外围空间通常满足 44px 有效触摸区域
> - Checkbox 的 16×16px 较小，依赖 Label 扩大可点击区域；独立使用时需注意触摸体验
> - Tabs 触发器已设置 `min-h-[44px]` 确保移动端可达
...
| Login blocked by unexpected MFA verification | Test user has orphaned TOTP credentials in the `credentials` table (type='totp') but `mfa_enabled=0` on the user record. The login flow detects TOTP credentials and triggers MFA verification even though MFA was not intentionally enabled. | Run `DELETE FROM credentials WHERE type='totp' AND user_id='...'` to remove orphaned TOTP credentials, or set `mfa_enabled=1` on the user record to match the credential state. Ensure `mfa_enabled` flag and credential rows are always consistent. |
