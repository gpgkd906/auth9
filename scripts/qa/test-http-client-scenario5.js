...
async function testScenario5() {
  ...
  try {
    ...
    // 2. 删除该租户
    console.log('\n2. 删除该租户...');
    const result = await client.delete(`/api/v1/tenants/${createdTenantId}`);
    ...
    // 4. 验证数据库状态
    console.log('\n4. 验证数据库状态...');
    console.log('请运行: SELECT id FROM tenants WHERE slug = \'to-delete-sdk-test\';');
    console.log('预期: 无记录');
